...
fn list_problems() -> Result<Vec<ProblemSummary>, String> {
    ...
    let entries = fs::read_dir(problems_dir()?).map_err(|error| format!("Could not read problems directory: {error}"))?;

    for entry in entries {
        let entry = entry.map_err(|error| format!("Could not read problem directory entry: {error}"))?;
...
#[tauri::command]
fn save_draft(app: tauri::AppHandle, problem_id: String, code: String) -> Result<SolutionDraft, String> {
...
fn execute_python(code: &str, tests: &ProblemTests, timeout_ms: u64) -> Result<Vec<TestResult>, String> {
    ...
    fs::write(&solution_path, code).map_err(|error| format!("Could not write solution: {error}"))?;
    fs::write(&runner_path, build_runner(tests)?).map_err(|error| format!("Could not write runner: {error}"))?;
    ...
    match child.wait_timeout(timeout).map_err(|error| error.to_string())? {
        Some(_) => {
            ...
            serde_json::from_str(stdout.trim()).map_err(|error| format!("Could not parse runner output: {error}"))
        }
        None => {
...
#[tauri::command]
fn run_tests(app: tauri::AppHandle, problem_id: String, code: String) -> Result<RunSummary, String> {
    let problem = read_problem(&problem_id)?;
    let started = Instant::now();
    let results = execute_python(&code, &problem.tests, problem.meta.time_limit_ms)?;
    let summary = RunSummary {
        passed: results.iter().filter(|result| matches!(result.status, TestStatus::Passed)).count(),
        failed: results.iter().filter(|result| !matches!(result.status, TestStatus::Passed)).count(),
